...
1) Start a browser and authenticate into the selected environment (DEV/PROD).
2) Visit each relevant ODM tab (Open Data in Europe 2024, Recommendations, Dimensions, Country profiles, Method and resources).
3) For each tab, download chart exports and related resources.
4) Validate the files against a manifest (expected_files.json).
...
- config.py — Central configuration and environment selection (DEV/PROD), URLs, credentials, and flags like HEADLESS. Also defines DOWNLOAD_DIR and EXPECTED_FILES_PATH.
- expected_files.json — Manifest of expected files used by the validator and reporter.
...
- src/ — Application package:
  ...
  - validator.py — Compares on‑disk files to expected_files.json and returns structured results.
...
- main.py calls:
  ...
  4. validator.validate_downloads(EXPECTED_FILES_PATH) — Compares disk artifacts with expected_files.json.
...
- downloader.py logic adapts to the current tab:
  - Open Data in Europe 2024: downloads charts and resources.
...
- ENVIRONMENT: "DEV" or "PROD" (default currently set to PROD). This selects the login URL and which environment variables to read.
...
- EXPECTED_FILES_PATH: Path to the expected files manifest (default: "expected_files.json").
...
- Validation failures:
  Review expected_files.json and the files produced under downloads/. Update the manifest if the source content legitimately changed.
